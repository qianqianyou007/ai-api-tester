...
=== 测试脚本生成规则 ===
1. 边界测试不要只写"参数为空"，至少包含一种数值边界（per_page=1、per_page=100）
2. expected_check 字段写具体可验证的内容，如"响应含login字段"，不要写"响应正常"这种模糊描述
3. expected_check 中如涉及字段名，使用实际 API 返回的字段名（参考 Few-shot 示例中的字段名）
4. GET 公开接口的场景中，expected_check 至少包含一个具体字段验证
